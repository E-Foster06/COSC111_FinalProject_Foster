# Project Title: Home library mangment software

## Author: Evelynn Foster
## Version: Fall 2025

### description: 
This is the final project for COSC 111 that demonstrates my mastery of objecs, classes, methods, and basic java concepts. It simulates a library application with a lot of functonality

### How to run my program: 
specific inputs, outputs, variables

### Short summary / features: 
This project has features including a menu, book handeling, checkout, and searching of a data base
